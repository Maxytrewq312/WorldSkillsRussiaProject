...
{
    public partial class Редактирование_профиля_сертификация_ : Form
    {
        ...
        public Редактирование_профиля_сертификация_()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (result == "ОК")
            {
                try
                {
                    ...
                    command = new SqlCommand($"update[dbo].[User] set Password = '{password}', FirstName = '{name}', LastName = '{lastName}' where Email = '{label7.Text}'", connectionSql);
                    command.ExecuteNonQuery();

                    command = new SqlCommand($"update[dbo].[Runner] set Gender = '{gender}', CountryCode = '{country}', DateOfBirth = CONVERT(datetime, '{data}', 4), Image = @img where Email = '{label7.Text}'", connectionSql);
                    command.Parameters.Add(new SqlParameter("@img", image));
                    command.ExecuteNonQuery();

                    DataRowView rowRunner = (DataRowView)comboBox3.SelectedItem;
                    command = new SqlCommand($"update[dbo].[Registration] set RegistrationStatusId = {rowRunner[0]} where RunnerId = (select RunnerId from Runner where Email = '{label7.Text}')", connectionSql);
                    command.ExecuteNonQuery();
            ...
            private void loadData() //Загрузка информации о бегуне
            {
                try
                {
                    ...
                    dataAdapter = new SqlDataAdapter($"SELECT dbo.Runner.Email, dbo.[User].FirstName, dbo.[User].LastName," +
                        ...
                        $"WHERE (dbo.Runner.Email = '{label7.Text}')", connectionSql);
                    DataSet dataSet = new DataSet();
                    dataAdapter.Fill(dataSet);
